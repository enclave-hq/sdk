// ZKPay Commitment Manager - Handles Commitment generation and submission process
...
class ZKPayCommitmentManager {
  constructor(walletManager, logger, options = {}) {
    ...
    this.defaultRecipientAddress =
      options.defaultRecipientAddress ||
   ...
   */
  async initialize() {
    this.logger.info("🔗 Initializing Commitment manager...");
    ...
    this.apiClient.interceptors.response.use(
      ...
      (error) => {
        ...
        this.logger.error(
          `❌ API response error: ${status} ${url} - ${error.message}`
        );
        return Promise.reject(error);
      }
    );
    ...
    this.logger.info("✅ Commitment manager initialization completed");
  }
  ...
  async getDeposit(chainId, localDepositId) {
    try {
      const response = await this.apiClient.get(
        `/api/v2/deposits/${chainId}/${localDepositId}`
      );
    ...
    while (Date.now() - startTime < maxWaitTime * 1000) {
      try {
        ...
        if (deposits.length > 0) {
          this.logger.info(
            `🔍 最新存款交易哈希: ${deposits[0].deposit_tx_hash}`
          );
        ...
        if (matchingDeposit) {
          ...
          this.logger.info(
            `   链上存款ID: ${matchingDeposit.local_deposit_id}`
          );
          return matchingDeposit;
        }

        this.logger.info(
          `⏳ 未找到匹配记录，等待${pollInterval / 1000}秒后重试...`
        );
        // 等待下次轮询
  ...
  async getUserDeposits(userAddress, chainId = null) {
    ...
    try {
      ...
      this.logger.info(
        `🔍 API查询参数: chain_id=${backendChainId}, owner_data=${ownerData}`
      );
      ...
      this.logger.info(
        `📊 查询结果: 找到 ${result.data ? result.data.length : 0} 条存款记录`
      );
      ...
      if (result.data && result.data.length > 0) {
        result.data.forEach((deposit, index) => {
          this.logger.info(
            `📋 存款 ${index + 1}: tx=${deposit.deposit_tx_hash}, status=${
              deposit.status
   ...
   */
  async performCommitmentFlowFromRecord(
    depositRecord,
    userAddress,
    wsToken = null
  ) {
    ...
    try {
      ...
      switch (depositRecord.status) {
        case "unsigned":
          this.logger.info(
            `✅ 存款已确认，等待状态变为ready_for_commitment...`
          );
          // 等待WebSocket推送状态变化到ready_for_commitment
          const updatedDeposit = await this.waitForDepositStatus(
            chainId,
            localDepositId,
          ...
          return await this.submitCommitmentV2WithDepositInfo(
            updatedDeposit,
            userAddress,
            "kms-demo-user"
          );
        ...
        case "issued":
          this.logger.info(
            `✅ Commitment已存在，状态: ${depositRecord.status}`
          );
          return {
  ...
  async waitForDepositStatus(
    ...
    while (Date.now() - startTime < maxWaitTime * 1000) {
      try {
        ...
        const latestDeposit = await this.getDepositByOwner(
          chainId,
          userAddress
        );
   ...
   */
  async submitCommitmentV2WithDepositInfo(
    depositRecord,
    userAddress,
    userName = "default"
  ) {
    ...
    try {
      ...
      const finalAmount =
        depositRecord.allocatable_amount || depositRecord.gross_amount; // 如果 allocatable_amount 为空，使用 gross_amount
      ...
      const requestData = {
        ...
        signature: {
          chain_id: targetChainId,
          signature_data: signature.startsWith("0x")
            ? signature
            : `0x${signature}`, // 🔧 修复：确保签名数据包含0x前缀
          public_key: null,
        },
        owner_address: {
          chain_id: targetChainId,
          address: AddressFormatter.toUniversalAddress(
            targetChainId,
            userAddress
          ), // 使用统一的地址格式化工具
        },
   ...
   */
  generateCommitmentSignatureMessage(
    depositRecord,
    recipientAddress,
    amount,
    targetChainId
    ...
    const ownerAddress = {
      chain_id: targetChainId,
      address: (
        depositRecord.owner?.data ||
        this.walletManager.getUserAddress("default")
      ).replace(/^0x/, ""),
    };
    const lang = 2; // 中文
  ...
  generateFullSignMessage(
    ...
    const formatUniversalAddress = (address, chainId) => {
      ...
      let chainAddress;
      console.log(
        `[DEBUG] formatUniversalAddress - address: ${JSON.stringify(
          address
        )}, type: ${typeof address}, chainId: ${chainId}`
      );
      ...
      if (
        ...
        console.log(
          `[DEBUG] 转换Universal Address: ${address} -> ${chainAddress}`
        );
      } else {
        // 这是普通地址，直接格式化
    ...
    message += `💰 您即将向 ${allocations.length} 位接收者分配 ${formatAmount(
      totalAmount
    )} ${tokenSymbol}：\n`;

    for (const allocation of allocations) {
      message += `  • ${formatUniversalAddress(
    ...
    message += `🔗 网络: ${this.getChainName(ownerAddress.chain_id)} (${
      ownerAddress.chain_id
    })\n`;
    message += `💎 代币: ${tokenSymbol} (ID: ${allocations[0].token_id})\n`;
    // 处理 owner 地址：根据链ID转换Universal Address为链特定格式
    const ownerChainAddress = this.convertUniversalAddressToChainSpecific(
      ownerAddress.address,
      ownerAddress.chain_id
    );

    message += `🔒 所有者: ${formatUniversalAddress(
      ownerChainAddress,
      ownerAddress.chain_id
    )}\n\n`;
  ...
  convertUniversalAddressToChainSpecific(universalAddress, chainId) {
    ...
    if (cleanAddress.length !== 64) {
      // 如果不是64字符，可能已经是链特定格式
      console.log(
        `[DEBUG] convertUniversalAddressToChainSpecific - universalAddress: ${JSON.stringify(
          universalAddress
        )}, type: ${typeof universalAddress}`
      );
      if (!universalAddress || typeof universalAddress !== "string") {
        throw new Error(
          `无效Universal Address: ${universalAddress}, 类型: ${typeof universalAddress}`
        );
      }
      return universalAddress.startsWith("0x")
        ? universalAddress
        : `0x${universalAddress}`;
    }
  ...
  async submitCommitment(checkbookId) {
    ...
    try {
      ...
      const response = await this.apiClient.post(
        "/api/v2/commitments/auto-execute",
        requestData
      );
      const result = response.data;
  ...
  async waitForCommitmentConfirmation(checkbookId, maxWaitTime = 300) {
    this.logger.info(
      `⏳ 等待Commitment确认 (最大等待时间: ${maxWaitTime}秒)...`
    );
    ...
    while (Date.now() - startTime < maxWaitTime * 1000) {
      try {
        ...
        if (error.response?.status === 404) {
          this.logger.debug(
            `📋 Checkbook ${checkbookId} 暂未创建，继续等待...`
          );
        } else {
  ...
  async performFullCommitmentFlow(depositData, recipientAddress) {
    ...
    try {
      ...
      results.generateProof = await this.generateCommitmentProof(
        checkbookId,
        recipientAddress
      );
  ...
  async createCheck(checkbookId, recipientInfo) {
    ...
    try {
      ...
      if (!tokenId) {
        const checkbookResponse = await this.apiClient.get(
          `/api/v2/checkbooks/${checkbookId}`
        );
  ...
  async getDepositByOwner(chainId, ownerAddress) {
    this.logger.debug(
      `🔍 通过owner查询存款: chain_id=${chainId}, owner=${ownerAddress}`
    );

    try {
      // 转换地址为universal格式
      const universalAddress = AddressFormatter.toUniversalAddress(
        chainId,
        ownerAddress
      );
      ...
      const response = await this.apiClient.get(
        `/api/v2/deposits/by-owner?${params.toString()}`
      );
      const result = response.data;
      ...
      this.logger.info(`✅ 获取到最新存款:`, {
        ID:
          latestDeposit.id ||
          `${latestDeposit.chain_id}-${latestDeposit.local_deposit_id}`,
  ...
  async getDepositStatus(chainId, localDepositId) {
    this.logger.debug(
      `🔍 获取存款状态: chain_id=${chainId}, local_deposit_id=${localDepositId}`
    );
  ...
  async getUserCheckbooks(userAddress, chainId = null) {
    try {
      ...
      const response = await this.apiClient.get("/api/v2/checkbooks", {
        params,
      });
      return response.data;
    } catch (error) {
