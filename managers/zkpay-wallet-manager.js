  ...
  async sendTransaction(transaction) {
    ...
    try {
      ...
      if (error.message && error.message.includes("broadcastTransaction")) {
        this.logger.warn(
          "⚠️ broadcastTransaction not available, trying sendTransaction"
        );
...
class ZKPayWalletManager {
  ...
  getRpcUrl(chainId) {
    ...
    if (defaultUrl) {
      if (actualChainId !== chainId) {
        this.logger.debug(
          `📡 SLIP44映射: ${chainId} -> ${actualChainId}, 使用RPC: ${defaultUrl}`
        );
      } else {
        this.logger.debug(
          `📡 使用默认RPC URL: Chain ${chainId} = ${defaultUrl}`
        );
      }
      return defaultUrl;
    }
    ...
    throw new Error(
      `未找到链 ${chainId} (实际链ID: ${actualChainId}) 的RPC URL，请设置环境变量 ${envVarName} 或使用支持的链ID`
    );
  }
  ...
  async initializeProviders() {
    ...
    for (const chainId of supportedChains) {
      try {
        ...
        this.logger.debug(
          `📡 链 ${chainId} RPC提供者已连接: ${rpcUrl} (实际链ID: ${network.chainId})`
        );
      } catch (error) {
    ...
    for (const [name, provider] of this.providers) {
      try {
        const network = await provider.getNetwork();
        this.logger.info(
          `🌐 ${name} 网络连接成功: Chain ID ${network.chainId}`
        );
      } catch (error) {
  ...
  async checkUserBalances(userName) {
    ...
    for (const [chainId, provider] of this.providers) {
      try {
        ...
        this.logger.info(
          `  📊 链 ${chainId}: ${ethers.formatEther(balance)} ETH`
        );
      } catch (error) {
  ...
  setUserSigner(userName, signer, address) {
    ...
    this.logger.info(
      `🔐 用户签名器已设置: ${userName} -> ${address} (KMS模式)`
    );
  }
  ...
  async waitForTransaction(txHash, chainId, confirmations = 1) {
    const provider = this.getProvider(chainId);
    this.logger.info(
      `⏳ 等待交易确认: ${txHash} (需要 ${confirmations} 个确认)`
    );
