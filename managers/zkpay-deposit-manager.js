// ZKPay Deposit Manager - Handles Token Approve and Deposit process

const { ethers } = require("ethers");
const { createLogger } = require("../utils/logger");

// ERC20 Token ABI (only includes required methods)
const ERC20_ABI = [
...
class ZKPayDepositManager {
  constructor(walletManager, logger, options = {}) {
    ...
    console.log(
      "  treasuryContracts:",
      Array.from(this.treasuryContracts.entries())
    );
  ...
  async initializeContracts() {
    ...
    for (const [chainId, treasuryAddress] of this.treasuryContracts) {
      try {
        ...
        this.logger.debug(
          `📜 Treasury合约已加载: 链 ${chainId} - ${treasuryAddress}`
        );
        ...
        for (const [tokenKey, tokenAddress] of this.tokenConfigs) {
          if (tokenKey.startsWith(`${chainId}_`)) {
            ...
            this.contracts.set(
              `token_${chainId}_${tokenSymbol}`,
              tokenContract
            );
            this.logger.debug(
              `🪙 Token合约已加载: 链 ${chainId} - ${tokenSymbol} - ${tokenAddress}`
            );
          }
        }
      } catch (error) {
  ...
  async approveToken(
    ...
    this.logger.info(
      `💰 当前余额: ${ethers.formatUnits(balance, decimals)} ${symbol}`
    );

    if (balance < amountWei) {
      throw new Error(
        `余额不足: 需要 ${amount} ${symbol}, 但只有 ${ethers.formatUnits(
          balance,
          decimals
        )} ${symbol}`
      );
  ...
  async executeDeposit(
    ...
    this.logger.info(
      `💰 开始执行存款: ${amount} ${tokenAddress} 在链 ${chainId}`
    );
    ...
    try {
      ...
      if (tokenId.toString() === "0") {
        throw new Error(
          `Token ${tokenSymbol} (${tokenAddress}) 不被Treasury合约支持`
        );
      }
      ...
      // 发送存款交易
      const tx = await treasuryContract.deposit(
        tokenAddress,
        amountWei,
        promoteCode,
        {
  ...
  async parseDepositEvent(receipt, treasuryContract) {
    try {
      ...
      for (let i = 0; i < receipt.logs.length; i++) {
        ...
        this.logger.info(
          `🔍 日志 ${i}: 地址=${log.address}, 主题数量=${log.topics.length}`
        );
          ...
          if (parsedLog) {
            ...
            if (log.topics.length > 0) {
              ...
              const expectedSignature =
                treasuryContract.interface.getEvent(
                  "DepositReceived"
                ).topicHash;
  ...
  async performFullDeposit(
    ...
    this.logger.info(
      `🚀 开始完整存款流程: ${amount} ${tokenAddress} 在链 ${chainId}`
    );
    ...
    try {
      ...
      this.logger.info(
        `🔍 调试: treasuryContracts类型:`,
        typeof this.treasuryContracts
      );
      this.logger.info(
        `🔍 调试: treasuryContracts大小:`,
        this.treasuryContracts.size
      );
      this.logger.info(
        `🔍 调试: treasuryContracts内容:`,
        Array.from(this.treasuryContracts.entries())
      );
      ...
      if (!treasuryAddress) {
        ...
        treasuryAddress = this.treasuryContracts.get(actualChainId);
        this.logger.info(
          `🔍 调试: SLIP44转换 ${chainId} -> ${actualChainId}, 找到Treasury: ${treasuryAddress}`
        );
      }
      ...
      if (!treasuryAddress) {
        throw new Error(
          `链 ${chainId} (实际链ID: ${this.walletManager.getActualChainId(
            chainId
          )}) 没有配置Treasury合约`
        );
      }
      ...
      this.logger.info(
        `🔍 当前授权额度: ${allowanceResult.formatted} ${allowanceResult.symbol}`
      );
  ...
  async performBatchDeposits(deposits, userName = "default") {
    ...
    for (let i = 0; i < deposits.length; i++) {
      ...
      this.logger.info(
        `📋 处理存款 ${i + 1}/${deposits.length}: ${deposit.amount} ${
          deposit.tokenSymbol
        } 在链 ${deposit.chainId}`
      );
    ...
    this.logger.info(
      `📊 批量存款完成: 成功 ${successCount}, 失败 ${failureCount}`
    );
    return {
      results,
      summary: {
  ...
  async getDepositHistory(
    ...
    try {
      ...
      const events = await treasuryContract.queryFilter(
        filter,
        fromBlock,
        toBlock
      );
