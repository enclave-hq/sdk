...
export class CommitmentFormatter {
  ...
  private static formatMessage(
    ...
    for (const allocation of allocations) {
      ...
      // Format decimal part (limit to max 6 decimal places, matching lib.rs {:.6})
      const maxDecimalDigits = 6;
      let formattedAmount: string;
      if (decimalPart === 0n) {
        formattedAmount = integerPart.toString();
      } else {
        // Pad to 18 digits first
        const decimalStr = decimalPart.toString().padStart(18, '0');

        // If we have more than 6 decimal places, round to 6 places
        if (decimalStr.length > maxDecimalDigits) {
          // Take first 6 digits and round based on 7th digit
          ...
          formattedAmount = trimmedDecimal
            ? `${integerPart}.${trimmedDecimal}`
            : integerPart.toString();
        } else {
          // Less than or equal to 6 digits, just trim trailing zeros
    ...
    // Format with max 6 decimal places (matching lib.rs {:.6})
    ...
    if (totalDecimalPart === 0n) {
      ...
      // If we have more than 6 decimal places, round to 6 places
      if (totalDecimalStr.length > maxDecimalDigits) {
        // Take first 6 digits and round based on 7th digit
        const first6Digits = totalDecimalStr.slice(0, maxDecimalDigits);
        const seventhDigit = parseInt(totalDecimalStr[maxDecimalDigits] || '0');
        const rounded6Digits =
          seventhDigit >= 5
            ? (BigInt(first6Digits) + 1n).toString().padStart(maxDecimalDigits, '0')
            : first6Digits;
        ...
        // Trim trailing zeros
        const trimmedDecimal = rounded6Digits.replace(/0+$/, '');
        totalFormatted = trimmedDecimal
          ? `${totalIntegerPart}.${trimmedDecimal}`
          : totalIntegerPart.toString();
      } else {
        // Less than or equal to 6 digits, just trim trailing zeros
        const trimmedDecimal = totalDecimalStr.replace(/0+$/, '');
        totalFormatted = trimmedDecimal
          ? `${totalIntegerPart}.${trimmedDecimal}`
          : totalIntegerPart.toString();
      }
