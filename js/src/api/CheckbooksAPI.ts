...
import type {
  ...
  GetCheckbookRequest,
  GetCheckbookByDepositRequest,
  GetCheckbookByDepositResponse,
} from '../types/api';
...
export class CheckbooksAPI {
  ...
  async getCheckbookById(request: GetCheckbookRequest): Promise<Checkbook> {
    ...
    if (response.data?.checkbook) {
      ...
      return this.processCheckbookData(checkbook, token);
    }
  ...
  /**
   * Get checkbook by deposit transaction
   * @param request - Get request with chain ID and transaction hash
   * @returns Checkbook data
   */
  async getCheckbookByDeposit(
    request: GetCheckbookByDepositRequest
  ): Promise<GetCheckbookByDepositResponse> {
    validateNonEmptyString(request.txHash, 'txHash');
    ...
    }>(`/api/checkbooks/by-deposit/${request.chainId}/${request.txHash}`);

    const processedCheckbook = this.processCheckbookData(response.checkbook, response.token);

    return {
      success: response.success,
      checkbook: processedCheckbook,
      token: processedCheckbook.token, // processedCheckbook now contains the correct token structure
    };
  }

  /**
   * Helper to process and normalize checkbook data from backend
   */
  private processCheckbookData(checkbook: any, tokenInfo?: any): Checkbook {
    ...
    const backendCheckbook = checkbook as any;

    // Log backend response for debugging
    console.log('📋 [CheckbooksAPI.processCheckbookData] Backend checkbook data:', {
    ...
    if (
      ...
      console.log(
        '✅ [CheckbooksAPI.processCheckbookData] Set localDepositId:',
        backendCheckbook.local_deposit_id
      );
    } else {
      console.warn(
        '⚠️ [CheckbooksAPI.processCheckbookData] Backend checkbook missing local_deposit_id:',
        {
    ...
    if (backendCheckbook.slip44_chain_id !== undefined) {
      ...
      if (!checkbook.token) {
        ...
        (checkbook as any).token = {
          ...
          iconUrl: undefined,
          isActive:
            tokenInfo?.is_active !== undefined ? Boolean(tokenInfo.is_active) : true,
          metrics: undefined,
        };
